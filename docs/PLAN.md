...
- Outcome: a few more wood grains and tile patterns in `src/core/textures.ts`, alongside the existing seven; and a room can scale the texture it uses, independently of every other room on the same texture, so a tile grid can be resized to match a real board width. The rotation half of this item is done — see S4.22, which shipped a per-room `textureRot` and the paint panel slider, on the same `texturePatterns`-per-`(id, rot)` mechanism this entry sketched.
- Files: `src/core/textures.ts` (more `wood`/`stone` entries, and `texturePatterns` taking a scale alongside the rotation it already carries), `src/core/schema.ts` (a room's texture reference grows `textureScale?: number`, alongside the existing `texture?`/`textureRot?`), `src/editor/panels.ts` (a scale control next to S4.22's rotation slider), `tests/core/textures.test.ts`, `tests/editor/editor.spec.ts`.
- Not started. Needs a design pass on the scale control (a slider like S4.22's, or a numeric field for a real board width) and which new wood/tile variants to add (Diego to pick, or a reasonable spread: herringbone/parquet wood, terracotta, checkerboard).
...
- Outcome (design only so far, not built): a door or window can attach more than one contact sensor, more than one vibration sensor, and more than one smart lock (currently `Door.sensor` is a single optional entity — becomes an array, `migrate()` wraps an old single value). A glass door or window gets its own dropdown for an electric curtain (`Door.cover`, already a free-text field, becomes a proper select restricted to `cover`-type catalog entries, shown only for `kind === "glass" | "window"`). A heater device can attach several TRV/climate entities and several temperature sensors (design interview, 2026-09-22: TRV + temp sensor only — no open-window cutoff, that was considered and declined). An AC device can attach several AC or TRV entities to one list. Diego confirmed (2026-09-22) every one of these is multi-attach, not the single-entity `bound` pattern the light already uses — so this is one reusable "attach several entities, filtered by type" panel component, not five bespoke ones.
- New `DeviceType`s needed: `lock` (smart door latch) and `vibration` (distinct from `motion` — a door/window vibration sensor is a different HA `device_class`, not room presence), each needing an icon in `src/core/icons.ts`, a `TYPE_LABELS` entry, and a default colour.
- Not started. Next: TDD from `src/core/schema.ts` outward (the array fields and `validate()`/`migrate()` cases first), then the shared multi-attach panel component, then wiring per surface (door panel, heater/AC device panels).
- Files (planned): `src/core/schema.ts` (`Door.sensors`/`vibration`/`locks: string[]`, `DEVICE_TYPES` gains `lock`/`vibration`, migration of the old singular `sensor`), `src/core/icons.ts`, `src/editor/state.ts` (a `bindMulti`-style method, one undo step), `src/editor/panels.ts` (the shared multi-attach component, used by `doorPanel` and a new heater/AC section of `devicePanel`), `tests/core/schema.test.ts` or equivalent, `tests/editor/editor.spec.ts`.
