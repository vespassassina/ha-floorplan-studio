...
### S7.16 The card reads the plan from the websocket reply
- Outcome: a card with no `layout` and no `layout_url` draws the plan the integration stores. It did not: the card passed the `{ layout }` wrapper to `migrate` and showed "No layout" on every real dashboard (`docs/DECISIONS.md`, 2026-09-25 S7.16).
- Done, 2026-09-25. Tests first, each watched to fail: `card.test.ts` websocket fallback mock now returns `{ layout }` as `websocket.py` does (failed against the old card), "layout: null shows the no-layout message", "an invalid layout says why"; `validate-cli.test.ts` "a floor saved before unlinked existed passes" and "a plan migrate refuses fails with its reason". Checked against the maintainer's stored plan through the CLI, outside the repo, then deleted.

### S7.15 Page scroll over a plan at fit; doors as label obstacles
- Outcome: two deferred review items (`docs/DECISIONS.md`, 2026-09-25 S7.15): a phone can scroll the dashboard from a plan at fit, and no room or zone name crosses a door.
- Done, 2026-09-25. Tests first, each watched to fail: `render.test.ts` "S7.15: moves off a door that runs through the centroid" and the stroke-edge pair (name stayed at the centroid before the fix), the demo clash test extended to doors (reported "Garden" and "Garden pond" on door 2); `card.spec.ts` "touch-action: pan-y at fit and none once zoomed" (computed-style pair) and "a vertical swipe on the plan scrolls the page at fit and pans the plan once zoomed" (CDP touch, page made 4000 px tall). Snapshots updated for the two garden names. Shots rendered and looked at: `card-ground-off-blueprint`, `editor-blueprint`, garden corner before and after.
...
### S8.2 Card sizing: fill the space instead of cropping
- Outcome: Diego reported the card cropped in Home Assistant's sections layout. `getGridOptions()` gives a numeric starting row count from the plan's aspect ratio (`columns: 12`, `min_columns: 6`, `min_rows: 3`); `:host` and `svg` both get `height: 100%`, matching Home Assistant's own thermostat and map cards, so a fixed-height row is filled and letterboxed instead of overflowing and being clipped, and an "auto" row is unchanged (`docs/DECISIONS.md`, 2026-09-25 S8.2).
- Done, 2026-09-25. Tests first: two Playwright tests in `card.spec.ts` (a fixed-height wrapper's svg fills it and never overflows — failed against the old CSS by 20 px before the fix; an unconstrained container is unchanged) and two unit tests in `card.test.ts` (`getGridOptions` shape and columns/min values; its `rows` changes with `layout.rotate`, same as `getCardSize`). Full suites green after the last edit: `npm run lint` exit 0; unit 1003/1003; `npm run build` exit 0; full Playwright 535/535 (1 skipped), exit 0; `npm run shots` identical to baseline, looked at `card-first-on-blueprint.png` by hand. Found and fixed two real regressions along the way, not papered over: `tests/card/harness.html` had the card as a direct child of `<body>`, so a test that sets `document.body.style.height` (S7.15, to give the page room to scroll) leaked straight into the new `height: 100%` and stretched the plan to the whole page — fixed by wrapping the card in a plain, unstyled div, matching how Home Assistant actually nests a card. Two rapid zoom-button taps followed immediately by a touch swipe also raced Chromium's compositor-thread commit of `touch-action: none` about 1 run in 10 (reproduced at `--repeat-each=20`, absent on `main` at the same rate over 40 runs); the S7.15 test now waits two rendered frames after the taps before swiping, a real rendering milestone, confirmed stable at `--repeat-each=30`.

### S8.3 Card: registry race and panel view height
- Outcome: the card defines itself again when HA's scoped-registry polyfill replaces `customElements` (it had lost the race on about half of Diego's hard reloads: "Custom element doesn't exist"), and in a panel view it takes the screen below the header instead of drawing the plan taller than the window (`docs/DECISIONS.md`, 2026-09-25 S8.3).
- Done, 2026-09-25. Diagnosed in Diego's Chrome on his HA: 2 of 4 hard reloads failed; the element was defined natively but the polyfill's `get` missed it; a second define on the new registry recovered the card live. Tests first: `tests/card/define.test.ts` (4 tests; the re-define one failed with the watch removed) and one Playwright test in `card.spec.ts` (panel caps the height at viewport minus 56 px, grid restores it; failed without the `:host([panel])` rule, green at `--repeat-each=5`). A second test that only checked the restore passed without the fix and was folded into the first. Panel CSS tried live on Diego's dashboard: card 846 px on a 902 px window, no page scroll. Then, at Diego's request, the card loads as a dashboard resource in storage mode (`panel.py`), `add_extra_js_url` only for YAML resources; 7 pytest tests in `test_panel.py`, each new one failing with its code removed (the filter test was strengthened after it passed with the filter gone). Released as 0.12.2.

### S8.4 Place popup starts empty; Select all / Deselect all
- Outcome: the room panel's Place popup opens with nothing ticked, not everything, so a click on the wrong row is never placed by accident; Place stays disabled at zero. A single `#placeAll` button replaces ticking rows off one at a time: it reads "Select all" while some shown row (respecting the type chip) is unticked and ticks every shown row, then flips to "Deselect all"; the button is disabled with nothing shown. Help text reads "Tick what to place" (`docs/DECISIONS.md`, 2026-09-25 S8.4/S8.5).
- Done, 2026-09-25. Tests first: two `editor.spec.ts` tests failed against the old `placeOff`-based popup (a ticked-by-default checkbox, no `#placeAll` locator) before `placeOn` and the toggle button existed. `npm test` 1015/1015; full Playwright suite green.

### S8.5 Add > Device becomes one floating panel with Floor / Room / Area / Type filters
- Outcome: Add's separate Device and Entities submenus merge into one `#addDevBtn` "Device…" that opens a floating, draggable `#addDevPanel` (search focused on open, Escape closes it) over every candidate: the catalog's unplaced entries and every HA entity not yet on the plan, one list, no duplicate (catalog wins an overlap). Four selects — Floor, Room, Area, Type — each list only the values present among candidates that pass the *other* active filters, narrowing as a filter is set; a final "None" option appears only when some filtered candidate lacks that field; a select with no value anywhere is hidden outright (no HA connected hides Area entirely). Floor and Room are plan concepts, found by matching an HA entity's area to the plan room that has it (any floor); Area is HA's own area name. Picking a candidate whose room lives on another floor switches to that floor first, so it lands in its own room; the panel stays open after a pick, so several devices go in one visit. `addCandidates(layout, ha)` in `src/core/ha.ts` is the pure function behind it (`docs/DECISIONS.md`, 2026-09-25 S8.4/S8.5).
- Done, 2026-09-25. Tests first: 8 `ha.test.ts` cases over every combination (catalog only, HA only, an overlap, no area, an area with no plan room, an area whose room is on floor 2, `ha: null`, an empty layout) — 4 failed with `locateEntity` stubbed to `return {}`. `editor.spec.ts`: rewrote ~20 references from the old `#mDev`/`#addEntSub`/`data-dev`/`data-ent` selectors to `#addDevPanel`/`data-add`, then added four more: select-narrowing with asymmetric data (Floor=First drops Living and every non-Climate type), the "None" option appearing only when a filtered candidate actually lacks the field, a cross-floor pick, and the search box holding focus on open. All four failed first: the floor/room/type narrowing and the None and cross-floor tests failed against `locateEntity` stubbed to `return {}`; the focus test failed for a real reason, not a test artefact — the toolbar's generic `onButtonClick` refocuses the host after *any* button click, same as it already did for `#addFloor` and `#helpClose`, so `#addDevBtn` needed the same exemption. New Playwright tests at `--repeat-each=10`: 80/80. Full suites green after the last edit: `npm run lint` exit 0; unit 1015/1015; `npm run build` exit 0; full Playwright 542/542 (1 skipped), exit 0; `.venv/bin/pytest tests/integration` 29/29; `npm run shots` identical to baseline, and the Add device panel looked at by hand (floor/area selects correctly absent with no HA connected, room/type selects present, rows grouped and muted with their room).

### S8.6 Devices, not entities, in every add list
- Outcome: Diego's own feedback: "in the device list i see plug network indicator and not the plug itself. just add the devices not the entities, and this applies everywhere." Add > Device, the room panel's Place popup, and a room's right-click "Add device from" now offer one row per HA device (`mainEntity`, `src/core/ha.ts`), not one per raw entity — a plug's switch, not its power sensor or diagnostic connectivity sensor. `mainEntity` drops any entity with a truthy `entity_category` ("config"/"diagnostic") first, then ranks what is left by domain (light > switch > climate > cover > fan > lock > media_player > vacuum > camera > binary_sensor > sensor > everything else); a device whose entities are all diagnostic gets no row. A device already placed through any one entity does not reappear through a sibling. `HaData` gained `devices` (the HA device registry) and `cat` on an entity, both optional so an older HA still works, grouping by the raw `dev` field and falling back to the main entity's own name. The one picker left alone is the already-placed device's own entity field (`deviceEntity`, `src/editor/panels.ts`), which still lists every entity — someone may want the power sensor once the icon exists — grouped under a `<optgroup>` per device within its existing tiers (`docs/DECISIONS.md`, 2026-09-25 S8.6).
- Done, 2026-09-25. Tests first: `ha.test.ts` `mainEntity`/`mainEntitiesByDevice` over a plug (switch + power + energy + diagnostic connectivity — main is the switch), a multisensor (motion + temperature + humidity + diagnostic battery — main is motion), a light bulb, and an all-diagnostic device (no row); `addCandidates` with a mocked device — one row, and a device placed via one entity does not reappear via a sibling. Proved the `cat` filter matters: commented it out, the all-diagnostic-device test failed (returned the config switch instead of undefined), restored, green. `hass-pickers.test.ts`: `haData` carries `name_by_user` over `name` for a device, and an entity's `entity_category` as `cat`; an HA with no device registry still returns, `devices` absent. `editor.spec.ts`: a plug device (switch + power + energy + diagnostic connectivity, one `dev` id) shows once as "Kitchen plug" in Add > Device, not once per entity, and places the switch — stable at `--repeat-each=10`. Full suites green: `npm run lint` exit 0; unit 1025/1025; `npm run build` exit 0; full Playwright 543/543 (1 skipped), exit 0.

### S8.7 Linking a light: floor switches, motion, auto-link
- Outcome: maintainer feedback: "when linking lights, only show the floor related switches, add also motion groups and motion sensors, or map them automatically, e.g. basement dumb light is managed by basement light switch." The light panel's "Controlled by" select is now floor-scoped (`switchChoicesForLight`, `src/core/ha.ts`) instead of listing every switch on the plan, with a unique same-area name-match candidate shown first, labelled "(suggested)". Edit, Group gained "Link lights to switches" (`EditorState.autoLinkLights`), which links every unbound light on the current floor to its suggested switch in one undo step. The select's own optgroup gained "Motion": picking a motion sensor or motion group never changes `bound`; it opens a "Turn on with X, off after N min, Create automation" row (the existing motion-group builder, generalised to also record the new `device.motion` field on the specific light, one undo step) and, once linked, an Unlink button that clears only `motion` — the automation itself is left in HA (`docs/DECISIONS.md`, 2026-09-25 S8.7).
- Done, 2026-09-25. Tests first: `schema.test.ts` "S8.7: a light's motion field" (5 cases, 3 failed with the old schema); `ha.test.ts` "switchChoicesForLight" (9 cases covering every suggestion combination in the brief — the "Basement dumb light"/"Basement light switch" 2-shared-token case, tied scores, sole-candidate, different area same floor, different floor, higher-score-wins, no-HA fallback, off-floor bound kept); `state.test.ts` `autoLinkLights` (links two lights on one floor in one undo step, leaves an already-bound light and a no-match floor untouched). New Playwright tests at `--repeat-each=10`: "Link lights to switches" (one click, one undo step reverts two lights), `boundField` floor-scoping and the suggested label, the Motion optgroup's writer gate and floor-only listing, and the full motion-link flow (pick leaves `bound` unchanged, Create automation posts the same config shape `motionLights` already built for the group flow, sets `motion` in one undo step, Unlink clears only `motion`) — all green, all failed first when temporarily disabled. Two existing `editor.spec.ts` tests ("choosing another free switch...", "a switch already bound elsewhere...") initially broke on the floor-scoped rewrite: they expected the old `{room} - {name}` label and catalog insertion order; `SwitchChoice` gained a `room` field and the unsuggested tier kept insertion order (no alphabetical sort) to match, both fixed without editing the tests. Full suites green after the last edit: `npm run lint` exit 0; unit 1041/1041; `npm run build` exit 0; full Playwright suite green (see the task report for the exact count).
