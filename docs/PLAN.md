...
### S4.19 More floor textures, and let a room rotate/scale its own (raised by Diego, 2026-09-22)
- Outcome: a few more wood grains and tile patterns in `src/core/textures.ts`, alongside the existing seven; and a room can rotate and scale the texture it uses, independently of every other room on the same texture, so a plank run or a tile grid can be turned to align with a wall that is not axis-aligned, or resized to match a real board width. Today `texturePatterns` renders one fixed `<pattern>` per texture id shared by every room using it — no per-room transform exists.
- Files: `src/core/textures.ts` (more `wood`/`stone` entries, and `texturePatterns` taking a per-room `patternTransform` instead of one shared pattern per id — likely one `<pattern>` per room-that-uses-a-texture rather than per texture id, so two rooms on the same texture can differ), `src/core/schema.ts` (a room's texture reference grows `rot?: number` and `scale?: number`, alongside the existing `texture?: string`), `src/editor/panels.ts` (rotate/scale controls next to the texture swatch picker, same place S4.9's length-lock checkbox sits next to its field), `tests/core/textures.test.ts`, `tests/editor/editor.spec.ts`.
- Not started. Needs a design pass on where rotate/scale live in the room panel and whether they reuse the plan's own rotate-by-45°-steps control (S1) or take a free value; and which new wood/tile variants to add (Diego to pick, or a reasonable spread: herringbone/parquet wood, terracotta, checkerboard).
