...
export class FloorplanStudioPanel extends LitElement {
  ...
  private async load() {
    ...
    try {
      ...
      this.layout = r.layout ?? emptyLayout();
    } catch (e) {
  ...
  render() {
    ...
    if (!this.layout) return html`<div class="msg">Loading…</div>`;
    return html`<floorplan-studio-editor
      .layout=${this.layout}
      .seed=${this.layout}
      .haDark=${this.hass?.themes?.darkMode}
      @save-request=${this.onSave}
    ></floorplan-studio-editor>`;
  }
