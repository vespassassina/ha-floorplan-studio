...
- **`domain`** / **`dc`**: the entity's domain (`light`, `switch`, `binary_sensor`, `sensor`, ...) and device class, when it has one (`motion`, `door`, `temperature`, ...). Use these to guess a sensible `Device.type` (`light`, `motion`, `contact`, `temp`, `person` for a `person.*` or `device_tracker.*` entity, `radar` for an mmWave presence sensor, `vacuum` for a `vacuum.*` entity, ...).
- **`person`** takes an optional `room` field: another entity id (never the person's own), a BLE room-presence sensor whose state, or `area_id`/`area` attribute, names one of the plan's rooms. Leave it out unless `available` (or the user) names that second entity too — never invent one.
- **`radar`** takes an optional `targets` field: a list of `{ "x": "<entity>", "y": "<entity>" }` pairs, one per tracked target, both entities from `available`. Leave it `[]` (or omit it) unless the user gives you the actual x/y sensor pairs.
